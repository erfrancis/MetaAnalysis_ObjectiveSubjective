...
---
title: 'Objective and subjective experiences of childhood adversity: a meta-analysis
  of their agreement and relationship with psychopathology'
author: "Emma Francis"
date: "19/06/2022"
---
...
forest(maltreat_z,
       ...
       xlim=c(-1.5, 1.5), at=c(-0.2,0,0.2,0.4,0.6,0.8), 
       xlab = "Correlation", 
       ...
       ilab.xpos=c(-1.5, -1.0), cex=0.4, 
       ilab.pos = c(4, 4),
       top=3) 
par(font=2)
text(c(-1.42, -0.9, 1.4), 43, c("Reference", "Maltreatment type", "Correlation [95% CI]"), cex=0.4)
```
...
forest(bully_z,
      ...
      ilab.xpos=c(-1.5,-0.8), cex=0.4, 
       ilab.pos = c(4, 4, 4),
       top=3) 
par(font=2)
text(c(-1.4, -0.68, 1.3), 21.5, c("Reference", "Bullying type", "Correlation [95% CI]"), cex=0.4)
```
...
# Forest plot
forest(neighbour_data$r_obj_subj[neighbour_data$es_type=="correlation"],
       vi=neighbour_data$r_obj_subj_var[neighbour_data$es_type=="correlation"], 
       transf=transf.ztor,
       order = order(neighbour_data$author[neighbour_data$es_type=="correlation"],
                     neighbour_data$year[neighbour_data$es_type=="correlation"]), 
       xlim=c(-1.5, 1), at=c(-0.2,0,0.2,0.4,0.6), 
       xlab = "Correlation", 
       mlab="Multivariate Meta-Analysis Model", 
       slab=NA, # Do not show references but add this manually below
       ilab=cbind(neighbour_data$author[neighbour_data$es_type=="correlation"], 
                  neighbour_data$year[neighbour_data$es_type=="correlation"], 
                  neighbour_data$Subj_Exposure[neighbour_data$es_type=="correlation"]), 
       ilab.xpos= c(-1.45, -1.23, -0.9), cex=0.4, 
       ilab.pos = c(4, 4, 4),
       top=2) 
par(font=2)
text(c(-1.37,-0.8, 0.87), 3.2, c("Reference", "Neighbourhood type", "Correlation [95% CI]"), cex=0.45)
```
...
plot<-forest(subj_neighbour2,
       ...
       slab=NA, 
       ilab=cbind(neighbour_data2$refPlot, neighbour_data2$Exposure_Type_Subj, neighbour_data2$MH_Outcome), 
       ilab.xpos=c(-1.92, -1.45, -1.0), 
       cex=0.35, 
       ilab.pos = c(4, 4, 4),
       textpos=c(-2, 1.2),
       top=2) 
par(font=2)
text(c(-1.8, -1.22, -0.8, 1.05), 3.3, c("Reference", "Neighbourhood type", "Outcome", "Correlation [95% CI]"), cex=0.4)
text(0, 3.3, "A. Subjective measure of neighbourhood", cex=.4, font=2)
...
forest(obj_neighbour2,
       ...
       top=2) 
par(font=2)
text(c(1.5), 3.3, c("Correlation [95% CI]"), cex=0.4)
text(0, 3.3, "B. Objective measure of neighbourhood", cex=.4, font=2)
```
...
bull_MH_subj<- rma.mv(z_r_subj, z_var_subj, random=list(~ 1 | es_id, ~ 1 | ref, ~ 1 | cohort_name),
                    data = data, mods= ~ MHoutcome_broad, subset = exposure_type_broad =="bullying" & !is.na(MHoutcome_broad) & MHoutcome_broad!="psychotic_experiences", slab=paste(ref, es_id, sep=", "))
bull_MH_subj

# Subjective measures,obtain effect size and ci 
bull_MH_subj<- rma.mv(z_r_subj, z_var_subj, random=list(~ 1 | es_id, ~ 1 | ref, ~ 1 | cohort_name),
                     data = data, mods= ~ MHoutcome_broad -1, subset = exposure_type_broad =="bullying" & !is.na(MHoutcome_broad) & MHoutcome_broad!="psychotic_experiences", slab=paste(ref, es_id, sep=", "))
bull_MH_subj
