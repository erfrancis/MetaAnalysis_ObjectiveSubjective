...
**Note: Corresponding data: "Data-extraction_agreement_20230126.xlsx"**
...
-   1.7 Is the agreement between subjective and objective measures moderated by the type of childhood adversity?

-   1.8 Publication bias

    -   1.8.1 Maltreatment - Fishers-Z
    -   1.8.2 Maltreatment - Kappa
    -   1.8.3 Bullying

-   1.9 Leave one out analysis

    -   1.9.1 Maltreatment - Kappa
    -   1.9.2 Maltreatment - Correlations
    -   1.9.3 Bullying
...
**Note: Corresponding data: "Data-extraction_20230126.xlsx"**
...
##### 1.1 Read in data, load packages and derive correlations
...
data <- data.frame(read_excel("/Users/emma/Desktop/Data-extraction_agreement_20230126.xlsx")) # Read dataset
str(data)
View(data)
...
forest(maltreat_z,
       ...
       ilab.xpos=c(-1.5, -1.1), cex=0.45, 
       ilab.pos = c(4, 4),
       top=1.5) 
par(font=2)
text(c(-1.38, -0.93, 1.3), 43, c("Reference", "Maltreatment type", "Correlation [95% CI]"), cex=0.45)
#save 575x600 
```
...
##### 1.7 Is the agreement between subjective and objective measures moderated by the type of childhood adversity?
```{r}
# Code exposure type specific variable indexing the type of maltreatment
data$exposure_type_specific[grepl("Sexual", data$Subj_Exposure, ignore.case = TRUE)] <- "Sexual_Abuse"
data$exposure_type_specific[grepl("Physical_Abuse", data$Subj_Exposure, ignore.case = TRUE)] <- "Physical_Abuse"
data$exposure_type_specific[grepl("Emotional", data$Subj_Exposure, ignore.case = TRUE)] <- "Emotional_Abuse"
data$exposure_type_specific[grepl("Psychological_Abuse", data$Subj_Exposure, ignore.case = TRUE)] <- "Emotional_Abuse"
data$exposure_type_specific[data$Subj_Exposure=="Family_Violence"] <- "Emotional_Abuse"
data$exposure_type_specific[data$Subj_Exposure=="Psychological_Safety_And_Security"]  <- "Emotional_Abuse"
data$exposure_type_specific[data$Subj_Exposure=="Acceptance_And_Self_Esteem"]  <- "Emotional_Abuse"
data$exposure_type_specific[data$Subj_Exposure=="Age_Appropriate_Autonomy"]  <- "Emotional_Abuse"
data$exposure_type_specific[data$Subj_Exposure=="Restriction"]  <- "Emotional_Abuse"
data$exposure_type_specific[grepl("Neglect", data$Subj_Exposure, ignore.case = TRUE)] <- "Neglect"
data$exposure_type_specific[grepl("Failure_To_Provide", data$Subj_Exposure, ignore.case = TRUE)] <- "Neglect"
data$exposure_type_specific[grepl("Lack_Of_Supervision", data$Subj_Exposure, ignore.case = TRUE)] <- "Neglect"
data$exposure_type_specific[grepl("Maltreatment_out_of_home_care", data$Subj_Exposure, ignore.case = TRUE)] <- "Maltreatment"
data$exposure_type_specific[grepl("Maltreatment_prior_to_out_of_home_care", data$Subj_Exposure, ignore.case = TRUE)] <- "Maltreatment"
data$exposure_type_specific[data$Subj_Exposure=="Maltreatment"] <- "Maltreatment"
data$exposure_type_specific[grepl("Neighbourhood", data$Subj_Exposure, ignore.case = TRUE)] <- "Neighbourhood"
data$exposure_type_specific[grepl("Neighborhood", data$Subj_Exposure, ignore.case = TRUE)] <- "Neighbourhood"
data$exposure_type_specific[grepl("Victimisation", data$Subj_Exposure, ignore.case = TRUE)] <- "Bullying"
table(data$exposure_type_specific)

# Compare original coding to new broad categories of adversity type
data %>% dplyr::select(author, Subj_Exposure, exposure_type_specific)

# initial test to get Q_moderation value and p-value
rma.mv(z_obj_subj, z_obj_subj_var, data=data, random=list(~ 1 | es_id, ~ 1 | author, ~1 | cohort),
                     mods=~exposure_type_specific)

# Look at the effect sizes for different maltreatment types
maltreat_spec <- rma.mv(z_obj_subj, z_obj_subj_var, data=data, random=list(~ 1 | es_id, ~ 1 | author, ~1 | cohort),
                     mods=~exposure_type_specific-1)

maltreat_spec

# Convert estimates and CIs back to r
round(fisherz2r(c(maltreat_spec$beta[1], maltreat_spec$ci.lb[1], maltreat_spec$ci.ub[1])),2) # bullying
round(fisherz2r(c(maltreat_spec$beta[2], maltreat_spec$ci.lb[2], maltreat_spec$ci.ub[2])),2) # emotional abuse
round(fisherz2r(c(maltreat_spec$beta[3], maltreat_spec$ci.lb[3], maltreat_spec$ci.ub[3])),2) # maltreatment broad
round(fisherz2r(c(maltreat_spec$beta[4], maltreat_spec$ci.lb[4], maltreat_spec$ci.ub[4])),2) # neglect
round(fisherz2r(c(maltreat_spec$beta[5], maltreat_spec$ci.lb[5], maltreat_spec$ci.ub[5])),2) # neighbourhood
round(fisherz2r(c(maltreat_spec$beta[6], maltreat_spec$ci.lb[6], maltreat_spec$ci.ub[6])),2) # physical abuse
round(fisherz2r(c(maltreat_spec$beta[7], maltreat_spec$ci.lb[7], maltreat_spec$ci.ub[7])),2) # sexual abuse

# Run pairwise comparison to see which exposures are significantly different from one another
mod_mal_type_spec <- rma.mv(z_obj_subj, z_obj_subj_var, data=data, random=list(~ 1 | es_id, ~ 1 | author, ~1 | cohort),
                            mods=~exposure_type_specific-1)

#install.packages("multcomp") # use for glht function
library(multcomp)
pairwise_mt_types <- summary(glht(mod_mal_type_spec, linfct=cbind(contrMat(rep(1,7), type="Tukey"))), test=adjusted("none"))
pvals <- pairwise_mt_types$test$pvalues # get p-values
pvals[pvals<=0.05] # Show significant pairwise differences
mod_mal_type_spec
# Level 7 (sexual abuse) is significantly different from level 1 (bullying), 2 (emotional abuse), 
# 3 (maltreatment composite), 4 (neglect),  5 (neighbourhood) and 6 (physical abuse)
```

##### 1.8 Test publication bias
...
# Read in data 
setwd(Data)
data <- data.frame(read_excel("Data-extraction_20230126.xlsx"))
head(data)
str(data)
```
...
forest(subj_maltreatment,
       ...
       ilab.xpos=c(-2.05, -1.6, -1.1), cex=0.35, 
       ilab.pos = c(4, 4, 4, 4),
       top=2) 
par(font=2)
text(c(-1.9, -1.4, -0.98, 1.34), 97, c("Reference", "Maltreatment type", "Outcome", "Correlation [95% CI]"), cex=0.4)
```
...
forest(obj_maltreatment2,
            ...
            # order studies from earliest to latest date
             ...
             top=2) 
      par(font=2)
      text(c(-2.05, -1.51, -1.05, 1.32), 97, c("Reference", "Maltreatment type", "Outcome", "Correlation [95% CI]"), cex=0.4)
```
...
forest(subj_bullying2, 
       ...
       top=2) 
par(font=2)
text(c(-1.85, -1.12, -0.56, 1.30), 47, c("Reference", "Bullying type", "Outcome", "Correlation [95% CI]"), cex=0.4)
```
...
forest(obj_bullying2,
      ...
      ilab=cbind(bullying_data2$refPlot, bullying_data2$Exposure_Type_Obj, bullying_data2$MH_Outcome), 
       ...
       top=3)
par(font=2)
text(c(-1.41, -0.89, -0.47, 0.78), 47, c("Reference", "Bullying type", "Outcome", "Correlation [95% CI]"), cex=0.4)
```
...
forest(subj_neighbour2,
       ...
       top=2) 
par(font=2)
text(c(-1.4, -0.92, -0.6, 0.75), 3.5, c("Reference", "Neighbourhood type", "Outcome", "Correlation [95% CI]"), cex=0.4)
```
...
forest(obj_neighbour2, 
       ...
       ilab.xpos=c(-1.52, -1.05, -0.6), cex=0.5, 
       ilab.pos = c(4, 4, 4, 4),
       top=3) 
par(font=2)
text(c(-1.38, -0.87, -0.48, 0.84), 3.5, c("Reference", "Neighbourhood type", "Outcome", "Correlation [95% CI]"), cex=0.5)
```
